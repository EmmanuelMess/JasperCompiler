...
Value print(ArgsType v, Compiler& e) {
	for (auto value : v)
		print(value);
	return e.null();
}
...
Value array_append(ArgsType v, Compiler& e) {
...
// arr1
Value array_extend(ArgsType v, Compiler& e) {
	// TODO proper error handling
	assert(v.size() == 2);
...
Value size(ArgsType v, Compiler& e) {
	// TODO proper error handling
	assert(v.size() == 1);
	Array* array = value_as<Array>(v[0]);
...
Value array_join(ArgsType v, Compiler& e) {
...
Value array_at(ArgsType v, Compiler& e) {
	// TODO proper error handling
	assert(v.size() == 2);
	Array* array = value_as<Array>(v[0]);
...
Value value_add(ArgsType v, Compiler& e) {
...
Value value_sub(ArgsType v, Compiler& e) {
	auto lhs = value_of(v[0]);
	auto rhs = value_of(v[1]);
...
Value value_mul(ArgsType v, Compiler& e) {
	auto lhs = value_of(v[0]);
	auto rhs = value_of(v[1]);
...
Value value_div(ArgsType v, Compiler& e) {
	auto lhs = value_of(v[0]);
	auto rhs = value_of(v[1]);
...
Value value_logicand(ArgsType v, Compiler& e) {
	auto lhs = value_of(v[0]);
	auto rhs = value_of(v[1]);
...
Value value_logicor(ArgsType v, Compiler& e) {
	auto lhs = value_of(v[0]);
	auto rhs = value_of(v[1]);
...
Value value_logicxor(ArgsType v, Compiler& e) {
	auto lhs = value_of(v[0]);
	auto rhs = value_of(v[1]);
...
Value value_equals(ArgsType v, Compiler& e) {
	auto lhs = value_of(v[0]);
	auto rhs = value_of(v[1]);
...
Value value_not_equals(ArgsType v, Compiler& e) {
	bool b = value_equals(v, e).as_boolean;
	return Value {bool(!b)};
}

Value value_less(ArgsType v, Compiler& e) {
	auto lhs = value_of(v[0]);
	auto rhs = value_of(v[1]);
...
Value value_greater_or_equal(ArgsType v, Compiler& e) {
	bool b = value_less(v, e).as_boolean;
	return Value {bool(!b)};
}

Value value_greater(ArgsType v, Compiler& e) {
	Value args[2] = {v[1], v[0]}; // arguments are swapped
	return value_less(Span<Value> {args, 2}, e);
}

Value value_less_or_equal(ArgsType v, Compiler& e) {
	bool b = value_greater(v, e).as_boolean;
	return Value {bool(!b)};
}

Value value_assign(ArgsType v, Compiler& e) {
	e.assign(v[0], v[1]);
	return e.null();
}

Value read_integer(ArgsType v, Compiler& e) {
	// TODO: error handling
	int result;
	std::cin >> result;
	return Value {result};
...
Value read_number(ArgsType v, Compiler& e) {
	// TODO: error handling
	float result;
	std::cin >> result;
	return Value {result};
...
Value read_line(ArgsType v, Compiler& e) {
	// TODO: error handling
	std::string result;
...
Value read_string(ArgsType v, Compiler& e) {
	// TODO: error handling
	std::string result;
	std::cin >> result;
	return Value {e.m_gc->new_string_raw(std::move(result))};
